...
private:
    ...
    std::string OnlyPath(std::string,std::string);
    uint32_t GetGraphDataPointer();
    bool CheckFileType();
    bool CheckHeaderData(uint8_t*);
    void GetPointers();
    void GetArgs();
    ...
    struct pointerAndLen{
        uint32_t pointer;
        uint32_t length;
    };
    static bool PointerCompare(const pointerAndLen&,const pointerAndLen&);
    static bool PointerSame(const pointerAndLen&,const pointerAndLen&);
...
//    const uint8_t mfSpritesNum=0xBF;
    ...
    std::vector<pointerAndLen>gfxPointers;
    std::vector<pointerAndLen>palPointers;
